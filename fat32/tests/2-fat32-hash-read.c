...
void notmain() {
  ...
  for (int i = 0; i < files.ndirents; i++) {
    ...
    read_and_hash(&fs, &root, "HELLO-F.BIN", 0x59f2a39a);

    // fill in the cksum values for these.
    // todo("fill in the checksum for these next three files (use `hash-files`)\n");
    read_and_hash(&fs, &root, "BOOTLO~5.BIN", 0xed7e8fc7);
    read_and_hash(&fs, &root, "CONFIG.TXT", 0x64bf62d4);
    read_and_hash(&fs, &root, "KERNEL.IMG", 0x641088df);
